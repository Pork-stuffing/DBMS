...
<body class="bootstrap-admin-with-small-navbar">
					...
					<div class="col-lg-12">
						<div class="panel panel-default bootstrap-admin-no-table-panel">
							...
							<div
								...
								<form class="form-horizontal" action="/book/book?method=listByPage"
									method="post">
	...
	<script type="text/javascript">
		...
		function deletebook(bid) {
			con = confirm("是否删除?");
			if(con == true) {
				location.href = "/book/book?method=delBook&bid=" + bid;
			}
		}
	</script>
	...
	<form class="form-horizontal" method="post"
		action="/book/book?method=updBook">
		<!--保证样式水平不混乱-->
	...
	<form class="form-horizontal" method="post" onsubmit="return checkAdd()"
		action="/book/book?method=addBook">
		<!--保证样式水平不混乱-->
		<!-- 模态框（Modal） -->
		...
		$("#addBookName").blur(function(){
			$.get("${pageContext.request.contextPath}/book?method=checkBook",{"bookName":$("#addBookName").val()},function(res){
				...
				if(obj.code==200){
					...
					$("#msg").html(obj.msg);
					$("#msg").css({"color":"red"});
					$("#aaddBookName").css({"border-color":"red"});
				}
			});
		});
